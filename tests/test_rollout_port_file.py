...
script stays correct on both backends without changing the established Slurm
text: Ray attempts use a per-job fallback and process-group cleanup, while
Slurm keeps ``$SLURM_JOB_ID`` and direct ``scancel``.
"""
...
class TestVllmPortFile:
    ...
    def test_default_preserves_slurm_job_id(self):
        """The default keeps the established Slurm-only path shape."""
        path = _vllm_port_file("/logs", "policy", "rs0_chunk0")
        assert path.endswith("_${SLURM_JOB_ID}.txt")
    ...
    def test_contains_per_seed_suffix(self):
        ...
        # Full established Slurm shape: dir / role / suffix / job id.
        assert path == "/logs/.vllm_port_policy_rs3_chunk1_${SLURM_JOB_ID}.txt"

    def test_attempt_id_makes_path_unique_per_attempt(self):
        """A per-attempt id is baked into the path so re-runs never collide.

        Under the Ray Jobs backend ``$SLURM_JOB_ID`` is constant for the head's whole
    ...
    def test_omitted_attempt_id_preserves_legacy_shape(self):
        ...
        path = _vllm_port_file("/logs", "policy", "rs0_chunk0")
        assert path == "/logs/.vllm_port_policy_rs0_chunk0_${SLURM_JOB_ID}.txt"
    ...
    def test_no_job_label_omits_suffix(self):
        """An empty label drops the suffix but keeps the Slurm job id."""
        path = _vllm_port_file("/logs", "policy", "")
        assert path == "/logs/.vllm_port_policy_${SLURM_JOB_ID}.txt"
...
class TestServerReaderPairing:
    ...
    Ray-backend re-run (constant ``$SLURM_JOB_ID``) cannot read a stale port.
    """
...
class TestClientCleanupBackendIsolation:
    """Cleanup rendering stays backend-specific."""

    def _build(self, *, is_ray: bool = False):
    ...
    def test_slurm_preserves_direct_scancel(self):
        """Slurm keeps its established direct cancellation command."""
        script = self._build()
        assert 'scancel "${SLURM_JOB_ID}"' in script
        assert "command -v scancel" not in script

    def test_ray_uses_process_group_kill_without_scancel(self):
        """A Ray job terminates its process group and never invokes Slurm."""
        script = self._build(is_ray=True)
        assert "kill 0" in script
        assert "scancel" not in script


class TestGymRayHeadAttach:
    """The gym must attach to the existing Ray cluster on the Ray Jobs backend.
...
class TestServerStopSentinel:
    ...
    differing attempts must yield different paths so a Ray-backend re-run never
