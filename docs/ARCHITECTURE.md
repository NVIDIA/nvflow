...
Dashed edges are optional. GRPO starts from the base HuggingFace checkpoint in the shipped configs (`hf_model_path: /hf_models/Qwen/Qwen3-4B`), so SFT is not a prerequisite for it — point `hf_model_path` at an SFT checkpoint only if you want to chain the two.
...
Stages:
  ...
  7. dgsdg_post_process                   - Clean + rename → final_result.jsonl
...
Model: Qwen3-4B dense (demo — equivalence_llm_judge: FSDP v2 @ 32K; finance_sec_search: Megatron TP2×CP8 @ 131K), Qwen3-30B-A3B MoE (production, Megatron)
```
...
graph TB
    subgraph "Finance Recipe Structure"
        ...
        subgraph "Stage modules (23 total)"
            direction LR
            SDG[SDG<br/>6 modules]
            RL[RL<br/>8 modules]
            SFT[SFT<br/>4 modules]
            Eval[Eval<br/>2 modules]
            DL[Download<br/>1 module]
            Shared[Shared<br/>2 modules]
        end
...
│  (nemo-rl/nemo-skills/vllm/vllm-grpo/nemo-gym built from        │
│   dockerfiles/*; only sglang pulled as-is — see INSTALL.md)     │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌──────────────────────┐  ┌──────────────────────┐             │
│  │ nvflow-nemo-skills   │  │ nvflow-vllm          │             │
│  │   (v1.1.2)           │  │   (v0.22.0)          │             │
│  │  SDG/eval/data prep  │  │  SDG/eval inference  │             │
│  └──────────────────────┘  └──────────────────────┘             │
│                                                                 │
│  ┌──────────────────────┐  ┌──────────────────────┐             │
│  │ sglang (pulled)      │  │ nvflow-nemo-rl       │             │
│  │   (v0.5.10.post1)    │  │   (v0.7.0)           │             │
│  │  SDG inference       │  │  SFT/GRPO trainer    │             │
│  └──────────────────────┘  └──────────────────────┘             │
│                                                                 │
│  ┌──────────────────────┐  ┌──────────────────────┐             │
│  │ nvflow-vllm          │  │ nvflow-nemo-gym      │             │
│  │   (v0.20.0 tag)      │  │   (0.4.0)            │             │
│  │ GRPO rollout/judge   │  │ CPU Gym-only stages  │             │
│  └──────────────────────┘  └──────────────────────┘             │
└─────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────┤
│  /workspace → writable data (outputs, HF cache)     │
│  /hf_models → /lustre/.../models/hf_models          │
│  /nemo_run/code → recipe code (nemo-run packaged)   │
└─────────────────────────────────────────────────────┘
```
...
```
<shared_filesystem>/workspace/nvflow/
│
...
- **Recipe → Workflow → Stage.** Stages are the unit of reuse and are discovered through the registry, so they can be shared across workflows and recipes, and listed without being hardcoded anywhere.
