...
> **Slurm (default) or Ray?** The recipes and data are shared, but setup and run commands differ. To run against **pre-provisioned Ray clusters**, follow [`INSTALL-RAY.md`](../../../INSTALL-RAY.md) (install with `--extra skills` → build/pull images → provision the Ray cluster → cluster config), then use the Ray run walkthrough [`quick-start-ray.md`](./quick-start-ray.md) — the Ray counterpart of [`quick-start.md`](./quick-start.md), with Ray-native fire/verify and absolute paths.
